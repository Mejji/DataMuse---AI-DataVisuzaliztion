...
export function SuggestionCard({ suggestion }: SuggestionCardProps) {
  ...
  return (
    <button
      ...
      className="group relative bg-white/80 backdrop-blur-sm border border-border/50 rounded-2xl p-5 text-left
                 hover:bg-white hover:border-dm-coral/30 hover:shadow-lg hover:shadow-dm-coral/5
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2">
        <h4 className="font-display font-bold text-dm-slate text-sm group-hover:text-dm-coral transition-colors duration-200 flex-1 pr-2">
          {suggestion.title}
        </h4>
        <div className="flex-shrink-0 w-7 h-7 rounded-lg bg-dm-coral-light flex items-center justify-center
